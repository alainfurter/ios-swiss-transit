...
- (void) commonInit {
    ...
    CGRect iconFrame = CGRectMake((self.frame.size.width - self.frame.size.height)/2, 0, self.frame.size.height, self.frame.size.height);
    normalIcon = [[UIImageView alloc] initWithFrame:iconFrame];
	selectedIcon = [[UIImageView alloc] initWithFrame:iconFrame];
    //highlightedIcon = [[UIImageView alloc] initWithFrame:iconFrame];
    ...
    normalIcon.contentMode = UIViewContentModeCenter;
    //highlightedIcon.contentMode = UIViewContentModeCenter;
	selectedIcon.contentMode = UIViewContentModeCenter;
...
- (void) layoutSubviews {
	...
	if (normalIcon.image != nil) {
        ...
        //highlightedIcon.layer.frame = CGRectMake(horizontalPadding, verticalPadding, imageSize, imageSize);
        
        //NSLog(@"Layout normal icon images");
        CGRect iconFrame = CGRectMake((self.frame.size.width - self.frame.size.height)/2, 0, self.frame.size.height, self.frame.size.height);
        normalIcon.frame =  iconFrame;
        selectedIcon.frame = iconFrame;
        //highlightedIcon.frame = iconFrame;
...
- (void) setHighlighted:(BOOL)highlighted {
	if (highlighted != self.highlighted) {
        ...
        //[self setSelected:highlighted animated:YES];
	}
}
...
- (void) configureViewPropertiesForControlState:(UIControlState)controlState {
    ...
    normalIcon.alpha = 1.0;
    selectedIcon.alpha = 1.0f - alpha;
    //highlightedIcon.alpha = 1.0f - alpha;
...
- (void) addGrayStyleForState:(UIControlState)state {
    ...
    */
    [self setColors:[NSArray arrayWithObjects:
                     [UIColor colorWithWhite:200.0f/255 alpha:1.0f],
                     [UIColor colorWithWhite:160.0f/255 alpha:1.0f],
                     nil] forControlState:state];
    ...
    [self setTextColor:[UIColor colorWithWhite:0.95f alpha:1.0f] forControlState:state];
