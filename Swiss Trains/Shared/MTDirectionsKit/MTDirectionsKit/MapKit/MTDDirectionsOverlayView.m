...
- (void)drawPath:(CGPathRef)path
     ...
     activeRoute:(BOOL)activeRoute
       ...
       zoomScale:(__unused  MKZoomScale)zoomScale
       inContext:(CGContextRef)context
       withColor:(UIColor *)routeColor {
        
    UIColor *baseColor = routeColor;
    CGFloat shadowAlpha = 0.4f;
    CGFloat secondNormalPathAlpha = 0.7f;
    CGFloat lineWidth = _fullLineWidth;
    UIColor *whitereplacementColor = [UIColor colorWithWhite: 0.5f alpha: 1.0];
    BOOL routeColorIsWhiteColor = [self colorsAreEqual: routeColor othercolor: [UIColor whiteColor]];
    ...
    UIColor *darkenedColor = MTDDarkenedColor(baseColor, 0.1f);
    if (routeColorIsWhiteColor) {
        darkenedColor = whitereplacementColor;
        darkPathLineWidth = darkPathLineWidth * 1.5f;
    }
...
- (UIColor *)convertColorToRGBSpace:(UIColor *)color {
    ...
    if(CGColorSpaceGetModel(CGColorGetColorSpace(color.CGColor)) == kCGColorSpaceModelMonochrome) {
        ...
        CGColorRef newcgcolor = CGColorCreate(colorSpaceRGB, components);
...
//AF ADDED
- (BOOL)colorsAreEqual:(UIColor *)basecolor othercolor:(UIColor *)otherColor {
    UIColor *selfColor = [self convertColorToRGBSpace:basecolor];
