...
// AF ADDED
- (void)loadRoutes:(NSArray *)routes zoomToShowDirections:(BOOL)zoomToShowDirections {
    [self mtd_loadRoutes: routes zoomToShowDirections: zoomToShowDirections];
}
...
// AF ADDED
- (void)mtd_loadRoutes:(NSArray *)routes zoomToShowDirections:(BOOL)zoomToShowDirections{
    ...
    [self mtd_notifyDelegateWillStartLoadingRoutes: routes];
    
    MTDDirectionsOverlay *overlay = [[MTDDirectionsOverlay alloc] initWithRoutes:routes
                                                               intermediateGoals:nil
...
// AF ADDED
- (void)mtd_notifyDelegateWillStartLoadingRoutes:(NSArray *)routes {
    id<MTDDirectionsDelegate> delegate = self.directionsDelegate;
    
    if (_directionsDelegateFlags.willStartLoadingDirections) {
        [delegate mapView:self willStartLoadingRoutes: routes];
    }
    
    NSNotification *notification = [NSNotification notificationWithName:MTDMapViewWillStartLoadingRoutes
                                                                 object:self
                                                               userInfo:nil];
    [[NSNotificationCenter defaultCenter] postNotification:notification];
}
