...
const CGFloat GCDiscreetNotificationViewBorderSizeiPad = 25;
const CGFloat GCDiscreetNotificationViewPaddingiPad = 5;
const CGFloat GCDiscreetNotificationViewHeightiPad = 30;

NSString* const GCShowAnimationiPad = @"show";
NSString* const GCHideAnimationiPad = @"hide";
NSString* const GCChangePropretyiPad = @"changeProprety";

NSString* const GCDiscreetNotificationViewTextKeyiPad = @"text";
NSString* const GCDiscreetNotificationViewActivityKeyiPad = @"activity";

@interface GCDiscreetNotificationViewiPad ()
...
@implementation GCDiscreetNotificationViewiPad
...
- (id)initWithText:(NSString*) text showActivity:(BOOL) activity inView:(UIView*) aView {
    return [self initWithText:text showActivity:activity inPresentationMode:GCDiscreetNotificationViewPresentationModeTopiPad inView:aView];
}

- (id) initWithText:(NSString *)text showActivity:(BOOL)activity 
 inPresentationMode:(GCDiscreetNotificationViewPresentationModeiPad)aPresentationMode inView:(UIView *)aView {
...
- (void) layoutSubviews {
    ...
    CGFloat baseWidth = (2 * GCDiscreetNotificationViewBorderSizeiPad) + (withActivity * GCDiscreetNotificationViewPaddingiPad);
    ...
    CGFloat textSizeWidth = 300;
    
    
    CGFloat activityIndicatorWidth = (self.activityIndicator != nil) ? self.activityIndicator.frame.size.width : 0;
    CGRect bounds = CGRectMake(0, 0, baseWidth + textSizeWidth + activityIndicatorWidth, GCDiscreetNotificationViewHeightiPad);
    if (!CGRectEqualToRect(self.bounds, bounds)) { //The bounds have changed...
        self.bounds = bounds;
        [self setNeedsDisplay];
    }
    
    if (self.activityIndicator == nil) self.label.frame = CGRectMake(GCDiscreetNotificationViewBorderSizeiPad, 0, textSizeWidth, GCDiscreetNotificationViewHeightiPad);
    else {
        self.activityIndicator.frame = CGRectMake(GCDiscreetNotificationViewBorderSizeiPad, GCDiscreetNotificationViewPaddingiPad, self.activityIndicator.frame.size.width, self.activityIndicator.frame.size.height);
        self.label.frame = CGRectMake(GCDiscreetNotificationViewBorderSizeiPad + GCDiscreetNotificationViewPaddingiPad + self.activityIndicator.frame.size.width, 0, textSizeWidth, GCDiscreetNotificationViewHeightiPad);
    }
...
- (void) drawRect:(CGRect)rect {
    ...
    if (self.presentationMode == GCDiscreetNotificationViewPresentationModeTopiPad) {
        maxY =  CGRectGetMinY(myFrame) - 1;
        minY = CGRectGetMaxY(myFrame);
    }
    else if (self.presentationMode == GCDiscreetNotificationViewPresentationModeBottomiPad) {
    ...
    CGPathAddCurveToPoint(path, NULL, CGRectGetMinX(myFrame) + GCDiscreetNotificationViewBorderSizeiPad, maxY, CGRectGetMinX(myFrame), minY, CGRectGetMinX(myFrame) + GCDiscreetNotificationViewBorderSizeiPad, minY);
    CGPathAddLineToPoint(path, NULL, CGRectGetMaxX(myFrame) - GCDiscreetNotificationViewBorderSizeiPad, minY);
    CGPathAddCurveToPoint(path, NULL, CGRectGetMaxX(myFrame), minY, CGRectGetMaxX(myFrame) - GCDiscreetNotificationViewBorderSizeiPad, maxY, CGRectGetMaxX(myFrame), maxY);
...
- (void) show:(BOOL)animated {
    [self show:animated name:GCShowAnimationiPad];
}

- (void) hide:(BOOL)animated {
    [self hide:animated name:GCHideAnimationiPad];
}
...
- (void) setOrigin:(CGRect)viewframe {
    CGRect frame = self.frame;
    CGPointMake(viewframe.size.width / 2 - 375 / 2,  -15);
    frame.origin = viewframe.origin;
    self.frame = frame;
    self.center = CGPointMake(viewframe.size.width / 2, -15 / 2);
}

- (void) showOrHide:(BOOL)hide animated:(BOOL)animated name:(NSString *)name {
    if ((hide && self.isShowing) || (!hide && !self.isShowing)) {
        
        //CGPoint hidecenter = CGPointMake(50, self.hidingCenter.y + 1);
        //NSLog(@"Center: %.1f, %.1f", self.hidingCenter.x, self.hidingCenter.y);
        //NSLog(@"Width: %.1f", self.frame.size.width);
        ...
        //self.center = hidecenter;
            ...
            //CGPoint showcenter = CGPointMake(self.showingCenter.x, self.showingCenter.y + 1);
            
            CGRect showframe = self.frame;
            showframe.origin.y = 34;
            self.frame = showframe;
            
            [self.activityIndicator startAnimating];
            //self.center = showcenter;
        }
...
- (void) animationDidStop:(NSString *)animationID finished:(BOOL) finished context:(void *) context {
    if (animationID == GCHideAnimationiPad) [self.activityIndicator stopAnimating];
    else if (animationID == GCChangePropretyiPad) {
        NSString* showName = GCShowAnimationiPad;
        
        for (NSString* key in [self.animationDict allKeys]) {
            if (key == GCDiscreetNotificationViewActivityKeyiPad) {
                self.showActivity = [[self.animationDict objectForKey:key] boolValue];
            }
            else if (key == GCDiscreetNotificationViewTextKeyiPad) {
    ...
    else if (animationID == GCShowAnimationiPad) {
        if (self.animationDict != nil) [self hide:YES name:GCChangePropretyiPad];
    }
...
- (void) setPresentationMode:(GCDiscreetNotificationViewPresentationModeiPad) newPresentationMode {
    if (presentationMode != newPresentationMode) {        
        BOOL showing = self.isShowing;
        
        presentationMode = newPresentationMode;
        if (presentationMode == GCDiscreetNotificationViewPresentationModeTopiPad) {
        ...
        else if (presentationMode == GCDiscreetNotificationViewPresentationModeBottomiPad) {
...
- (CGPoint) showingCenter {
    CGFloat y = 0;
    if (self.presentationMode == GCDiscreetNotificationViewPresentationModeTopiPad) y = 15;
    else if (self.presentationMode == GCDiscreetNotificationViewPresentationModeBottomiPad) y = self.view.frame.size.height - 15;
...
- (CGPoint) hidingCenter {
    CGFloat y = 0;
    if (self.presentationMode == GCDiscreetNotificationViewPresentationModeTopiPad) y = - 15;
    else if (self.presentationMode == GCDiscreetNotificationViewPresentationModeBottomiPad) y = 15 + self.view.frame.size.height;
    return CGPointMake(self.view.frame.size.width / 2, y);
}
...
- (void) setTextLabel:(NSString *)aText animated:(BOOL)animated {
    if (animated && (self.showing || self.animating)) {
        [self changePropretyAnimatedWithKeys:[NSArray arrayWithObject:GCDiscreetNotificationViewTextKeyiPad]
...
- (void) setShowActivity:(BOOL)activity animated:(BOOL)animated {
    if (animated && (self.showing || self.animating)) {
        [self changePropretyAnimatedWithKeys:[NSArray arrayWithObject:GCDiscreetNotificationViewActivityKeyiPad]
...
- (void) setTextLabel:(NSString *)aText andSetShowActivity:(BOOL)activity animated:(BOOL)animated {
    if (animated && (self.showing || self.animating)) {
        [self changePropretyAnimatedWithKeys:[NSArray arrayWithObjects:GCDiscreetNotificationViewTextKeyiPad, GCDiscreetNotificationViewActivityKeyiPad, nil]
...
- (void) changePropretyAnimatedWithKeys:(NSArray*) keys values:(NSArray*) values {
    ...
    if (!self.animating) [self hide:YES name:GCChangePropretyiPad];
}
